...
public interface CategoryMapper extends BaseMapper<Category> {
    ...
    @Select("select " + COLUMNS + " from category c LEFT JOIN category c2 on c.id=c2.parent_id where c2.id=#{childId} limit 1") // #{参数名} 插入参数
    @ResultMap("BaseResultMap")
    Category getByChildId(@Param("childId") Long childId); // 定义参数
}
