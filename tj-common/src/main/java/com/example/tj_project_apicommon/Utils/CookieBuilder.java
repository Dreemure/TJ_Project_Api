...
/*
 * Cookie 构建器。
 * 职责：链式构建 Cookie，自动处理 domain 推导、UTF-8 URL 编码（防中文乱码），并写入 HttpServletResponse。
 * 使用：new CookieBuilder(request, response).setName("token").setValue("xxx").build()。
 * 注意：若未显式指定 domain，则从请求的 serverName 自动推导（取二级域名）。
 */
@Slf4j
@Data
