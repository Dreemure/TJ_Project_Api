...
public class TreeDataUtils {
     ...
     *
     * @param data     树形数据列表
     * @param processor 计算处理器
     * @return 聚合结果；data 为空时返回 null
     */
    public static <T> Object ergodicTreeCalculate(List<T> data, CalculateDataProcessor<T> processor) {
        if (CollUtils.isEmpty(data)) {
            return null;
        }
        Object result = null;
        for (var item : data) {
            var selfData = processor.getData(item);
            var childData = processor.getChildData(item);
            var childResult = ergodicTreeCalculate(childData, processor);
            var currentAllData = processor.calculate(selfData, childResult);
            processor.setResult(item, currentAllData);
            result = processor.calculate(result, currentAllData);
        }
        return result;
    }
